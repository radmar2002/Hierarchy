...
path_enum <- setRefClass(
    ...
    methods = list(
        ...
        descendants_ids = function(path, end = max(count_occ(data()[[.path]], .sep)), start = 1, include = FALSE) {
            ...
            x <- if (length(x) > 0) as.character(x) else NULL  # fix to "Error in x[sapply(x, function(i) { : invalid subscript type 'list'"
        ...
        # Endnodes methods (endnode is the last node in the subtree)
